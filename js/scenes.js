...
BATO.OptionsScene = {
  ...
  create(){
    ...
    lhToggle.on('pointerdown',()=>{
      opts.leftHand = !opts.leftHand;
      lhVal.setText(opts.leftHand?'ON':'OFF');
      BATO.Save.save();
    });
...
BATO.CreditsScene = {
  ...
  create(){
    ...
    let credits = [
      'BATO: Perks of Being a Senator',
      '',
      'Game Design & Development',
      'Based on the hit concept',
      '',
      'All enemies, bosses, perks,',
    ...
    credits.forEach((l,i)=>{
      this.add.text(w/2, 80+i*16, l, {fontSize:'9px',color:'#aaaaaa',fontFamily:'monospace'}).setOrigin(0.5);
    });
    let back = this.add.text(w/2, h-30, 'BACK', {fontSize:'14px',color:'#ffffff',fontFamily:'monospace'}).setOrigin(0.5).setInteractive();
    back.on('pointerover',()=>back.setColor('#ffcc00'));
    back.on('pointerout',()=>back.setColor('#ffffff'));
  ...
  create(){
    ...
    chars.forEach(c=>{
      ...
      if(c.unlocked){
        box.on('pointerdown',()=>{
          BATO.AudioManager.playSfx('ui_confirm');
          this.scene.start('PerkLoadoutScene',{character:c.id});
        });
      }
    });

    if(sanUnlocked){
      this.add.text(w/2, h-80, 'San Bato unlocked!', {fontSize:'8px',color:'#ffd700',fontFamily:'monospace'}).setOrigin(0.5);
    }
...
BATO.PerkLoadoutScene = {
  key:'PerkLoadoutScene',
  init(data){ this.character = data.character || 'bato'; },
  ...
  create(){
    ...
    slots.forEach((s,i)=>{
      this.add.text(40, 60+i*25, s.label, {fontSize:'9px',color:'#ffcc00',fontFamily:'monospace'});
      let slotBox = this.add.rectangle(120, 66+i*25, 120, 20, 0x444444).setInteractive();
      let slotTxt = this.add.text(125, 60+i*25, 'Empty', {fontSize:'8px',color:'#888888',fontFamily:'monospace'});
      slotBox.setData('slot', i);
      slotBox.setData('txt', slotTxt);

      slotBox.on('pointerdown',()=>{
        this.showPerkPicker(activePerks, (perk)=>{
          BATO.Perks.activeSlots[i] = perk;
          slotTxt.setText(perk.name.substring(0,16));
        });
      });
    });

    this.add.text(40, 150, 'Passive', {fontSize:'9px',color:'#aa88ff',fontFamily:'monospace'});
    let passBox = this.add.rectangle(120, 156, 120, 20, 0x444444).setInteractive();
    let passTxt = this.add.text(125, 150, 'Empty', {fontSize:'8px',color:'#888888',fontFamily:'monospace'});
    passBox.on('pointerdown',()=>{
      this.showPerkPicker(passivePerks, (perk)=>{
        BATO.Perks.passiveSlot = perk;
        passTxt.setText(perk.name.substring(0,16));
      });
    });
    ...
    start.on('pointerdown',()=>{
      let loadout = {
        active: BATO.Perks.activeSlots.filter(p=>p!==null).map(p=>p.id),
        passive: BATO.Perks.passiveSlot ? BATO.Perks.passiveSlot.id : null
      };
      ...
      this.scene.start('CutsceneScene',{
        dialogueKey:'intro',
        nextScene:'GameScene',
        nextData:{character:this.character, zone:0, stage:0, loadout:loadout}
      });
    });
  ...
  create(){
    ...
    items.forEach((item,i)=>{
      ...
      buy.on('pointerdown',()=>{
        if(BATO.Save.spendCoins(item.cost)){
          ...
          this.shopBoughtEffects = this.shopBoughtEffects || [];
    ...
    proceed.on('pointerdown',()=>{
      this.scene.start('GameScene',{
        character:this.character, zone:this.zone, stage:this.stage,
...
BATO.GameScene = {
  ...
  spawnPlayer(){
    ...
    if(this.boughtEffects.includes('armor_up')){
      this.player.maxHp = 125;
      this.player.hp = 125;
    }
    if(this.boughtEffects.includes('full_heal')){
    ...
    this.enemies.getChildren().forEach(e=>{
      if(e.active && e.hp>0){
        ...
        if(dist < 50){
          ...
          if(knockbackAll) e.body.setVelocityX(dir*200);
        }
      }
    });
    if(this.boss && this.boss.active && Math.abs(this.boss.x-player.x)<80){
      this.boss.hp -= dmg;
    }
  },
  ...
  onProjectileHitEnemy(proj, enemy){
    if(proj.active && enemy.active && enemy.hp>0){
      ...
      let dir = proj.body.velocity.x > 0 ? 1 : -1;
      enemy.takeDamage(dmg, dir);
      if(enemy.getData('holy')){}
      proj.destroy();
    }
  },
  ...
  onPlayerTouchEnemy(player, enemy){
    if(!player.active || !enemy.active || enemy.hp<=0) return;
    if(player.getData('invincible')) return;
    if(enemy.getData('converted')) return;
    let now = this.time.now;
    if(enemy._lastHit && now-enemy._lastHit<500) return;
    enemy._lastHit = now;
    let dmg = enemy.baseDmg || 5;
    let died = player.takeDamage(dmg);
    if(died) this.onPlayerDeath();
  },

  onPlayerDeath(){
    if(this.gameOver) return;
    if(BATO.Perks.passiveSlot && BATO.Perks.passiveSlot.id==='coffin_dance'){
      this.player.hp = 1;
      BATO.Perks.passiveSlot = null;
      BATO.AudioManager.playSfx('heal');
      this.showAchievement('revived');
      return;
    }
    this.gameOver = true;
    BATO.Save.save();
    ...
    BATO.AudioManager.stopMusic();
    let w=this.scale.width, h=this.scale.height;
    let deathText = this.add.text(w/2, h/2, 'GAME OVER', {fontSize:'24px',color:'#ff4444',fontFamily:'monospace'}).setOrigin(0.5).setDepth(500).setScrollFactor(0);
    let lost = Math.floor(BATO.Save.getCoins()*0.5);
    BATO.Save.data.coins -= lost;
    BATO.Save.save();
    let coinsLost = this.add.text(w/2, h/2+30, 'Lost P'+lost, {fontSize:'12px',color:'#ffcc00',fontFamily:'monospace'}).setOrigin(0.5).setDepth(500).setScrollFactor(0);
  ...
  update(time, delta){
    ...
    BATO.Prayer.add(0); // passive fill handled by hits
    if(this.player.attackTimer>0) this.player.attackTimer-=delta;
...
BATO.BossScene = {
  ...
  create(){
    ...
    for(let x=0;x<800;x+=16){
      let g = this.platforms.create(x, 250, null).setDisplaySize(16,16).setVisible(false);
      g.body.updateFromGameObject();
    }

    let bx=400, by=190;
    this.bossSprite = this.add.sprite(bx, by, 'boss_'+bossData.id).setScale(1.5);
    ...
    let bossLabel = this.add.text(w/2, 10, bossData.name, {fontSize:'10px',color:'#ff4444',fontFamily:'monospace'}).setOrigin(0.5).setDepth(100);
    let hpBarBg = this.add.rectangle(w/2, 22, 200, 8, 0x440000).setDepth(100).setScrollFactor(0);
  ...
  onPlayerTouchBoss(player, boss){
    ...
    this._bossHit = now;
    let died = player.takeDamage(10);
    if(died) this.onPlayerDeath();
  },

  updateBossPhase(){
    let pct = this.bossHp/this.bossMaxHp;
    if(pct<=0.66 && this.bossPhase===0){
      this.bossPhase=1;
      if(this.bossData.phases[1]) this.bossPhaseLabel.setText(this.bossData.phases[1].name);
    }
    if(pct<=0.33 && this.bossPhase===1){
      this.bossPhase=2;
      if(this.bossData.phases[2]) this.bossPhaseLabel.setText(this.bossData.phases[2].name);
    }
  ...
  bossAttack(){
    ...
    switch(atk.type){
      ...
      case 'wave':
        ...
        if(Math.abs(this.player.x-this.bossSprite.x)<100) this.player.takeDamage(atk.dmg);
        break;
      case 'charge':
        this.tweens.add({targets:this.bossSprite,x:this.player.x<this.bossSprite.x?-50:850,duration:800,
          onComplete:()=>{ if(this.player && Math.abs(this.player.x-this.bossSprite.x)<50) this.player.takeDamage(atk.dmg); }});
        break;
      case 'spawn':
        for(let i=0;i<3;i++){
          let e = BATO.EnemyFactory.createEnemy(this, 'shadow_hooded', this.bossSprite.x+BATO.Utils.rand(-50,50), 200);
          if(e) this.enemies.add(e);
        }
        break;
      case 'projectile':
        for(let i=0;i<5;i++){
          let p = this.physics.add.sprite(this.bossSprite.x, this.bossSprite.y, 'proj_enemy');
          p.setTint(0xff6666);
      ...
      case 'laser_slow':
        ...
        this.time.delayedCall(500,()=>{if(this.player) this.player.takeDamage(atk.dmg); laser.destroy(); });
        break;
      case 'barrel_rain':
        for(let i=0;i<6;i++){
          let bx = this.bossSprite.x+BATO.Utils.rand(-100,100);
          let barrel = this.add.circle(bx, -20, 10, 0x885522);
          this.tweens.add({targets:barrel,y:300,duration:1000+(i*200),
            onComplete:()=>{ if(this.player && Math.abs(this.player.x-barrel.x)<20) this.player.takeDamage(atk.dmg); barrel.destroy(); }
          });
        }
        break;
      case 'throw':
        let tree = this.add.rectangle(this.bossSprite.x-20, this.bossSprite.y-10, 60, 12, 0x448833);
        let tx = this.player.x<this.bossSprite.x?-100:900;
        this.tweens.add({targets:tree,x:tx,y:300,duration:700,onComplete:()=>{if(this.player&&Math.abs(this.player.x-tree.x)<40)this.player.takeDamage(atk.dmg);tree.destroy();}});
        break;
      default:
        if(Math.abs(this.player.x-this.bossSprite.x)<100) this.player.takeDamage(atk.dmg);
    }
  ...
  onPlayerDeath(){
    if(this.gameOver) return;
    if(BATO.Perks.passiveSlot && BATO.Perks.passiveSlot.id==='coffin_dance'){
      this.player.hp=1; BATO.Perks.passiveSlot=null; return;
    }
    this.gameOver=true;
    BATO.AudioManager.playSfx('death');
    BATO.AudioManager.stopMusic();
    let w=this.scale.width;
    this.add.text(w/2, 135, 'GAME OVER', {fontSize:'24px',color:'#ff4444',fontFamily:'monospace'}).setOrigin(0.5).setDepth(500);
    let lost = Math.floor(BATO.Save.getCoins()*0.5);
    BATO.Save.data.coins -= lost;
    BATO.Save.save();
  ...
  onBossDefeated(){
    ...
    if(this.bossData && this.bossData.id === 'diktador'){
      this.onTrueEnding();
      return;
    }
  ...
  onTrueEnding(){
    ...
    this.add.text(400, 120, 'Saving...', {fontSize:'10px',color:'#ffffff',fontFamily:'monospace'}).setOrigin(0.5).setDepth(200);
  ...
  checkEnding(){
    ...
    if(heatZero && isSanBato){
      BATO.Save.data.sanBatoUnlocked = true;
      BATO.Achievements.check('divine_comedy', this.game);
    }

    let endingKey = 'ending_normal';
    if(heatZero) endingKey = 'ending_secret';

    this.time.delayedCall(3000,()=>{
      this.scene.start('CutsceneScene',{
        dialogueKey:endingKey,
        nextScene:'MenuScene'
      });
    });
  },
  ...
  update(time, delta){
    ...
    this.enemies.getChildren().forEach(e=>{
      if(e.updateEnemy) e.updateEnemy(time, delta, this.player);
    });

    this.handlePerkInput();

    if(this.bossHp>0){
      if(time-this.bossAttackTimer>2000){
